...
- __block-but__: filtrage par formation (code de BUT) proposé par l'IUT dont au moins un des parcours proposé est oubert à l'alternance (tient compte de la casse). Paramètre répétable jusqu'à 20 occurences. Si plusieurs formations sont fournis, le service filtre les IUT qui proposent au moins une des formation données, dont au moins un des parcours proposé est ouvert à l'alternance (OU logique inclusif). Ce paramètre est incompatible avec le paramètre block.
