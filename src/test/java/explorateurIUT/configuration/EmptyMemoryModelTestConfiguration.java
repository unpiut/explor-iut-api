 ...
 */
@TestConfiguration
public class EmptyMemoryModelTestConfiguration {

    private static final Log LOG = LogFactory.getLog(EmptyMemoryModelTestConfiguration.class);

    @Bean
    public BUTIUTModelManager butIUTModelManager(Validator validator) {
        LOG.info("Initiate TEST EMPTY but iut model manager...");
        final BUTIUTModelManagerImpl modelManager = new BUTIUTModelManagerImpl(validator);
        BUTIUTModel newEmptyModel = modelManager.startNewModelCreation();
