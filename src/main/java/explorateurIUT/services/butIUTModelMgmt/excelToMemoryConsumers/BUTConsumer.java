...
import explorateurIUT.services.butIUTModelMgmt.excelImport.model.ExcelParcoursBUT;
...
public class BUTConsumer implements Consumer<ExcelBUT> {
    ...
    public BUTConsumer(BUTIUTModel model) {
        this.model = model;
    }
    ...
    public void accept(ExcelBUT excelBut) {
        ...
        final BUT but = model.saveBUT(butFromExcel(excelBut));
        butByCode.put(but.getCode(), but);
        excelBut.getParcours().forEach((excelParcours) -> {
            LOG.debug("- Save Parcours " + excelParcours.getCode());
            ParcoursBUT parcours = model.saveParcours(parcoursFromExcel(but, excelParcours));
